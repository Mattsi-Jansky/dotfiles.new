...
def test_all_already_installed():
    r = Runner()
    with patch("framework.providers.apt.runner", r):
        from framework.providers.apt import install_apt_packages
        install_apt_packages("G", ["curl", "git"])

    with patch("framework.providers.apt.run") as mock_run:
...
def test_installs_missing_packages():
    ...
    with patch("framework.providers.apt.run", side_effect=mock_run_side_effect):
        outcomes = _outcomes(r)
